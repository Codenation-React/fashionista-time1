...
const ProductModal = (props) => {
  ...
  useEffect(() => {
    if (state.products.length > 0) {
      return setProduct(state.products.find(item => {
    ...
    axios.get(catalogUrl)
      .then(response => {
        setProduct(response.data.find(item => {
          return formatText(item.name) === props.match.params.id;
        }));
      })
      .catch(error => {
  ...
  if (product.sizes) {
    btns = product.sizes.map(sku => {
      return <SizeButton key={sku.sku} disabled={!sku.available} size={sku.size} onClickHandler={() => addProduct(sku.size)} />
    })
  }
